...
> 状态（9.28 末）：**地板 + 贴墙 + 贴天花板三形态均已接入**（网格/命中/放置/渲染/拆除/频道/Lua）。**遗留问题「天花板 button/toggle 模块模型向 N 偏 1px」已修复并复验**（2026-09-24，见文末「遗留」；根因 = 天花板 Rx(180) 翻转绕模型原点角而非足迹中心 → 足迹中心恒偏 N 1px；修复 = 翻转绕足迹中心，四朝向进游戏复验通过）。**E/W 非方形屏幕内容偏移已按转置语义修复并复验通过**（2026-09-24，见 §5；逻辑格子按正面视角定义 + 渲染转置可绘制区，满框 1px 字形）。**扳手整拆改为返还普通 monitor_slab 物品（不保留 BE 数据/NBT，与手动破坏一致）**（2026-09-24 用户拍板，见「实施记录·扳手整拆返还普通物品」）。参考来源：monitor_2 完整接入记录见 `memo/control-desk-grid-slot.md`（MonitorGridHost 三件套复用、命中自遮挡坑、px 单位移植坑）。
...
- ~~贴墙 / 贴天花板变体的模块放置~~（**9.28 已接入**；遗留天花板模块 N 偏 1px 已修复并复验通过，见文末「遗留」）
- Flywheel Visual（性能优化）
- Sable physics_block_properties、合成配方

---

## 实施记录（2026-09-24，扳手整拆返还普通物品）

- **需求（用户拍板）**：扳手潜行右键整拆的 monitor_slab 应与手动破坏一致——**不保留 BE 数据/NBT，
  返还普通 monitor_slab 物品**（原行为：BE 数据存进物品 `BLOCK_ENTITY_DATA`，重放保留频道/GridState）。
- **改动**：`block/MonitorSlabBlock.java` `onSneakWrenched` 删掉「saveAdditional + addEntityType +
  `DataComponents.BLOCK_ENTITY_DATA`」嵌入逻辑，直接返还 `new ItemStack(this)`（普通物品）。
  其余语义不变：点击表面内容 → 放行拆单模块/屏幕；已装内容 → 禁止整拆提示
  （`gui.ccpe.monitor_slab.remove_blocked`）；光板 → 整拆返还普通物品。
- **影响**：扳手拆下的物品不再保留频道/网格状态（重放为新 slab、频道自动分配）；与手动破坏
  （`getDrops` 掉普通 slab + 模块/屏幕单独掉落）在「空 slab」上完全一致；已装内容的 slab 仍不允许
  扳手整拆（须先拆模块或手动破坏）。
- **验证**：gradlew classes 编译通过；待用户进游戏复验（扳手蹲下右键光板 slab → 背包得到普通
  monitor_slab、无 NBT 提示；重放后是新 slab 无旧频道；已装内容仍提示先拆模块）。
...
### 2. 天花板旋钮拖拽方向反了（已修复并复验通过 ✅，2026-09-24）
...
- **验证**：✅ 2026-09-24 用户进游戏复验通过（天花板拖拽方向与地板一致，地板/贴墙不回归）。

### 3. 天花板 slab 放置方向绕 Y 转 180°（用户拍板，2026-09-24 已修复并复验通过 ✅）
...
- **验证**：✅ 2026-09-24 用户进游戏复验通过（天花板 slab 放置后内容朝向符合需求，地板不回归）。

### 4. 天花板屏幕字符水平镜像（整行顺序反 + 每个字符左右反，2026-09-24 已修复并复验通过 ✅）
...
- **修复**：`MonitorSlabRenderer` 天花板屏幕外层 `scale(1,1,−1)` → **`scale(−1,1,−1)`**（X 一起反射 →
  ...
  模型/字形绕序均不需再改。
- **验证**：✅ 2026-09-24 用户进游戏复验通过（N/S 字符不再镜像、E/W 不再偏移，文字正常）。

### 4b. 天花板屏幕外框整体飞到西边邻格（用户实测，2026-09-24 已修复并复验通过 ✅）

- **现象（用户实测）**：修完 4 后字符方向正确了，但**屏幕外框（9 宫格模型）整体飞到西边邻格并镜像**
  （网格 (0,0)-(5,5) 的屏幕出现在西边邻格 (9,0)-(14,5)，文字相对框内位置正确）。任务 1 诊断：选 c（对角区域）。
- **根因（推导确认）**：`scale(−1,1,−1)` 在屏幕外层变换里是绕**面板西缘（x=0）**反射（transform 原点 = 面板原点），
  不是绕屏幕中心 → 整个屏幕（外框+文字）被翻到面板西边邻格。
- **修复**：把反射枢轴移到**每个屏幕中心**——`T(cx,cy,0)·scale(-1,1,-1)·T(-cx,-cy,0)`
  （cx/cy = 屏幕区域中心，块单位）。推导：content (x,y,z) → world (2cx−x, z+8/16, y)，X 绕 cx 反射 → 屏幕留在
  原位、内容水平回正、z 不变。
- **验证**：✅ 2026-09-24 用户进游戏复验通过（屏幕外框留在原网格位置、不飞西边邻格）。

### 5. 地板 E/W 朝向非方形屏幕内容偏移（**已按转置语义修复，用户进游戏复验通过 ✅，2026-09-24**）

#### 现象（用户实测，任务 2/3 + 后续确认）

- 地板 **east / west** 朝向的 slab 上，**非方形**屏幕内容有偏移，**只与屏幕宽高比有关**：
  - 方形（5×5）→ 正常；
  - 3×5（窄高）→ 偏东南；
  - 5×3（宽矮）→ 偏西北；
  - **east 与 west 都有**（用户确认；最初只报 west 是因为 east 上没放过 3×5）。
- north / south 朝向**任何宽高比都正常**（用户确认"其他方向正常"）。

#### 根因（推导确认）

- 屏幕**内容**跟随朝向做平面内旋转（`Screen9GridRenderer.applyInPlaneRotation` 绕屏幕区域中心转内容、
  9 宫格**外框不旋转**——9.27 的修复），inPlane：north 0° / east 90° / south 180° / west 270°。
- **旋转 90°/270° 会把内容外接框的宽高互换**（3×5 → 5×3），与不旋转的 3×5 外框对不上 → 内容溢出/偏移；
  0°/180° 旋转**不改变长方形外接框宽高**（转半圈还是 3×5）→ N/S 永远正常；方形旋转 90° 外接框不变 → 正常。
- 所以问题只出现在 **E/W + 非方形** 组合；不是 west 独有，east 一样（对称问题）。

#### 几何约束（为什么不能"无代价"重新划分 cells）

屏幕区域固定为 W×H（如 3 格宽 × 5 格高）。内容旋转 90° 后文字在面板上需要"5 列沿 Z、3 行沿 X"，
但外框只有 3 格 X 方向空间 → **旋转后内容总有一个轴向放不下**，重新划分 cells 必然二选一：

| 方案 | 效果 | 代价 |
|---|---|---|
| **A 等比缩小适配**（推荐） | 旋转跟随朝向 + 格子等比缩小适配框内；字不变形、始终在框内、正面可读 | 字变小、四周留白边 |
| **B 拉伸填满** | 旋转跟随朝向 + 格子拉伸填满外框 | 格子变长方形、字被压扁/拉长 |
| **C 不旋转** | 内容不跟随朝向、字不变形不缩小、填满框 | 东西朝向从正面看文字是侧着的（朝向语义不一致） |

方形屏幕不受影响（旋转后外接框不变），三种方案对 N/S 都零影响。

#### 相关参考（实现落点）

- `block/Screen9GridRenderer.java`：`ScreenPlane.inPlaneRotationDeg()`（内容平面内旋转角，默认 0）、
  `applyInPlaneRotation`（绕屏幕区域中心 `(cx,cy)` 旋转内容；`cx = plane.originX() + (minX+maxX+1)/2·cell`）。
  - 方案 A/B 的实现落点：在 `applyInPlaneRotation` 之后（或 slabPlane 侧）补一个**等比/非等比缩放**，
    把旋转后的内容外接框适配到屏幕区域外框。缩放需按 inPlane 90/270 时内容外接框 = (H×W) 计算：
    A：`scale = min(W/H, H/W)` 等比；B：`scale = (W/H, H/W)` 非等比（绕屏幕中心缩放，与旋转同一枢轴）。
  - 方案 C 的实现落点：`MonitorSlabRenderer.screenInPlaneDeg` / `slabPlane` 对非方形屏幕返回 inPlane=0
    （需在 slabPlane 拿到 scr 的宽高判断，或在 overlay 放置时就限制）。
- `block/MonitorSlabRenderer.java`：`screenInPlaneDeg(state, face)`（floor = facingInPlaneDeg：E90/S180/W270/N0）、
...
- **验证**：✅ 2026-09-24 用户进游戏复验通过：E/W 非方形屏幕内容满框、1px 字形、正面可读、贴角不居中
  （不再小字 + 白边）；N/S、方形、贴墙屏幕不回归；天花板 E/W 一并确认正常。
...
> - ✅（2026-09-24 后续修复，见文末「遗留」：Rx(180) 翻转绕足迹中心，**四朝向进游戏复验通过**）
...
### 遗留：天花板模块 N 偏 1px（**2026-09-24 已修复**，待进游戏复验）
...
- **根因（2026-09-24 推导确认）**：天花板分支变换链为
  `T(anchor) · Rx(180) · T(pivot) · Ry(facing) · T(−pivot)`（pivot = 足迹中心 (0.5,0,0.5)）。
  **Rx(180) 直接绕锚点（= 模型原点角）翻转**，而不是绕足迹中心：翻转后足迹中心落在
  `anchor + Rx(180)(pivot) = (px+0.5px, ·, pz−0.5px)`，应为 `anchor + pivot = (px+0.5px, ·, pz+0.5px)` →
  **恒偏 N（−Z）1px，与 facing 无关**（north/south/east/west 都偏 N，符合「偏 N 是否恒为 N」推断）。
  knob 枢轴为 0 → 翻转绕中心天然成立 → 正常（对照成立）。
- **修复（2026-09-24）**：`block/MonitorSlabRenderer.java` 天花板分支把 Rx(180) 用 pivot 平移对包住，
  翻转绕足迹中心进行：`T(anchor) · T(pivot) · Rx(180) · T(−pivot) · T(pivot) · Ry(facing) · T(−pivot)`
  （= `T(anchor) · T(pivot) · Rx(180) · Ry(facing) · T(−pivot)`）。
  **旋转部分 `Rx(180)·Ry(facing)` 与修复前完全相同** → 标签 up/right 四朝向语义不变；
  平移部分相对旧实现 = 纯 +1px（+Z 南移），足迹中心回到格心 (px+0.5, ·, pz+0.5)。
  逐顶点验算（facing north/south）：toggle 底座 x∈[1.1,1.9]/16、z∈[1.1,1.9]/16 居中于格 (0,0)
  （[1,2]/16）；button 底座 y∈[7.875,8]/16 背面贴面板（8/16）、头 z∈[0.625,1]px → 凸出朝下，全部正确。
- ⚠️ ~~待进游戏复验：N/S/E/W 四朝向各放 button/toggle 于天花板 slab，确认模型居中于网格、不再偏 N 1px；~~
  ~~同时确认标签/按压/旋钮拖拽/屏幕渲染不回归（旋转未变，理论上零回归）。~~
  **✅ 2026-09-24 用户进游戏复验：四朝向 button/toggle 均居中于网格、不再偏 N 1px，修复完成。**
...
- [x] 天花板 slab：网格/放置/屏幕已正常（✅ 9.28 末确认）；button/toggle 模型 N 偏 1px（❌ 遗留）
  → **2026-09-24 已修复（翻转绕足迹中心）并复验通过：四朝向 button/toggle 均居中于网格，不再偏 N 1px**
...
- [ ] 旋钮拖拽方向手感（`atan2(pz−cz, px−cx)` + renderExtra `Axis.YP −anim`；若反了翻转 atan2 符号）
  → **2026-09-24：天花板已修并复验通过**（`MonitorSlabGridOverlay` 拖拽角按 FACE==CEILING 取反，见 9.24 追加记录；用户确认拖拽方向与地板一致，地板/贴墙不回归）
