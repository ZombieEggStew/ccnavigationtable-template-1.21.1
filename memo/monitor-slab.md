...
> 状态（9.28 末）：**地板 + 贴墙 + 贴天花板三形态均已接入**（网格/命中/放置/渲染/拆除/频道/Lua）。**遗留问题「天花板 button/toggle 模块模型向 N 偏 1px」已修复并复验**（2026-09-24，见文末「遗留」；根因 = 天花板 Rx(180) 翻转绕模型原点角而非足迹中心 → 足迹中心恒偏 N 1px；修复 = 翻转绕足迹中心，四朝向进游戏复验通过）。参考来源：monitor_2 完整接入记录见 `memo/control-desk-grid-slot.md`（MonitorGridHost 三件套复用、命中自遮挡坑、px 单位移植坑）。
...
### 4b. 天花板屏幕外框整体飞到西边邻格（用户实测，2026-09-24 修复，待复验）
