...
| `block/EngineCoreBlockEntity.java` | 发动机核心 BE：`GeneratingKineticBlockEntity` 骨架，**当前占位不发电**（`getGeneratedSpeed()` = `GENERATED_SPEED` = 0，被动节点、贯通传动杆随网络连通）；后续接入燃烧/蒸汽模块时改转速并调 `updateGeneratedRotation()`；**静态工具 `engineControllerPos(level, pos)`**：从任一模块方块（核心成员/燃烧室/冷却风道）解析整条引擎 controller 坐标（未连接核心返回 null），供模块 goggle tooltip 代理；**客户端活塞"噗嗤"音效池 ×2**（`fluidPistonSoundPool`/`steamPistonSoundPool`，`SoundPool(4,2)` 懒创建，音量各自走 `Config.ENGINE_*_PUFF_VOLUME`，`tickClient` 每 tick `play()`，燃烧室按类型入队；音量=0 跳过） | 发动机核心发电逻辑接入 |
