...
| `block/EngineCoreBlockEntity.java` | 发动机核心 BE：`GeneratingKineticBlockEntity` + `IMultiBlockEntityContainer`（P0 组网：controller/length/LastKnownPos/Uninitialized 重组，`getMaxLength=21`、宽度固定 1）；**P1–P3 controller tick**（扫描燃烧室 → 逐室评估 → 发电 256rpm / 容量 = 室数×base×stress×效率 / 零缓存 capability drain / 牛顿冷却温度 + 过热滞回 + 冲压/气压冷却 / 效率同缩出力·热量·消耗，非 controller `getGeneratedSpeed()` 恒 0）；**P4 Lua 外设 `ccpe:engine`**（`getPeripheral()` 非 controller 委托给 controller；内嵌 `Peripheral`：`getTemperature`/`isOverheated`/`getFluidTanks`/`setEnabled`/`getEnabled`/`setThrottle`/`getThrottle` + `attach`/`detach` 维护 `luaConnected`；`enabled` 默认 true、NBT 持久化、throttle=0 停机；注册见 `compat/cc/CCPeripheralCapabilities`）；**静态工具 `engineControllerPos(level, pos)`**：从任一模块方块（核心成员/燃烧室/冷却风道）解析整条引擎 controller 坐标（未连接核心返回 null），供模块 goggle tooltip 代理；**客户端活塞"噗嗤"音效池 ×2**（`fluidPistonSoundPool`/`steamPistonSoundPool`，`SoundPool(4,2)` 懒创建，音量各自走 `Config.ENGINE_*_PUFF_VOLUME`，`tickClient` 每 tick `play()`，燃烧室按类型入队；音量=0 跳过）；Goggle 首行标题 `tooltip.ccpe.engine.header` + 状态行（正常/即将过热/过热）+ 温度趋势外推显示 + 效率行 + Lua 控制连接行（`luaConnected`，attach/detach 维护） | 发动机核心发电/冷却/Lua 控制逻辑 |
