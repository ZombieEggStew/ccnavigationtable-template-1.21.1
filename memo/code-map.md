...
| `block/CoolingDuctBlock.java` | 冷却风道（`ccpe:cooling_duct`）：**6 面贴附 × 每面 2 旋转 = 12 个 blockstate**（`FACING` + `AXIS_ALONG_FIRST`(布尔)，结构参考 PitotTubeBlock / rope_connector 的 `AbstractDirectionalAxisBlock`）；**双模型**：地板/天花板/墙面 axis=true 用平模型 `block.json`（y 旋转出南北/东西、墙面叠 x:90 贴墙 = 第二旋转），墙面 axis=false 用竖模型 `block_v.json`（竖直贴墙，绕 Y 4 向）；放置：地板/天花板按玩家水平朝向定 axis、墙面恒 false；纯静态方块（无 BE，物品模型 = block 模型）；IWrenchable（照 rope_connector）：右键贴附面（= FACING）翻转 axis（任何朝向通用）、其他面走 Create 默认（换贴面/转朝向）、潜行右键拆除；音效对齐发动机核心（SoundType.NETHERITE_BLOCK）；**实现 `IProxyHoveringInformation`**（无 BE 也显示 goggle tooltip：经 `EngineCoreBlockEntity.engineControllerPos` 代理到整条引擎 controller，代理时记录悬停方块 → **气道显示专属精简 tooltip（温度 + 效率=风门 setCooling%）**；未连接核心返回自身 → 无 BE 不显示） | 风道朝向/碰撞、blockstate `assets/ccpe/blockstates/cooling_duct.json`、竖模型 `assets/ccpe/models/block/aero_engine/cooling_duct/block_v.json` |
