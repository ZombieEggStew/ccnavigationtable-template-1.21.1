...
| `compat/cc/ThrottleModuleHandle.java` | 油门杆模块实例：前进/后退按住态 `isForwardActive/isBackActive`（读服务端输入租约）+ 档位 `getThrottleGear()`（0..11 整数，锁存不回正；自由模式下 = 就近档位）+ 轴值 `getAxis()`（0..1，档位模式离散 / 自由模式连续）+ 自由模式开关 `isFreeMode/setFreeMode`（mainThread=true，NBT 持久化，无 GUI）+ 直接写位置 `setAxis`（0..1，mainThread=true）；读取 mainThread=false |
