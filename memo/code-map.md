...
| `compat/cc/SensorSystemAPI.java` | `ccpe.sensor_system` Lua API：所在物理体（含约束链）环境数据（静压孔高度/气压、皮托管速度/空速、**INS 姿态 getAngles()、位置 getPosition()（INS 世界坐标）、机体原点 getBodyPosition()（世界坐标）、四元数 getOrientation()、角速度 getAngularVelocity()——均门控 = 机体上有 ≥1 INS**；**物理数据 getPhysicsCenterOfMassRel()（重心相对最后放置的 FMC，含 AIC，plot 帧差值）、getPhysicsMass()/getPhysicsChainMass()（kg）、getPhysicsGravityForce()/getPhysicsChainGravityForce()（pN = 质量×11）——门控 = 机体上有 ≥1 FMC（飞行管理计算机，ccpe:fmc）**；**螺旋桨转速工具 initPropeller(N,S) + getPropellerRPM(F,P,V,θ?)（门控 = FMC；公式 R = F/(P·S^1.5·N·T) + V·sinθ/(S^0.5·A)，T/A 从 aeronautics 配置 Propeller Bearing Thrust/Airflow 静态缓存，默认 0.2/0.05，进游戏（ServerStartingEvent）与放置/加载 FMC（FmcBlockEntity.onLoad）时刷新一次，不逐 tick 读）**；**物理体频道寻址 getPeripheral(ch)（getLinker 过滤：链接器 → 附着外设；控制台 → 控制台外设，经 ControlDeskRegistry 查同一注册表）+ 链接器红石三件套 getRedstoneOutput/getRedstoneInput/setRedstoneOutput**）；每电脑 tick `update()` 主线程刷新 volatile 缓存（姿态 pitch/roll/yaw 度 = gimbal_sensor 重力投影 + 世界北水平方位；INS/机体原点世界坐标经 `SableCompat.projectOutOfSubLevel` / `getSubLevelWorldPos`；四元数/角速度/质量/质心直接读 SableCompat），Lua 方法 mainThread=false 直读 |
