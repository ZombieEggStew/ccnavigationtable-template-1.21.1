...
| `compat/cc/ControlDeskPeripheral.java` | 控制台的 `IPeripheral` 实现（`getType()` = `"ccpe:control_desk"`；`getModule("pedal"/"joystick"/"joystick_2"/"throttle")` 返回控件模块实例，未安装返回 nil；**`getModule("monitor")` → `MonitorPeripheral`（type = "ccpe:monitor_2"）——monitor_2 表面小 Monitor 的模块/屏幕查询入口，方法同 Monitor 外设**；**`getMonitor2Module(id)` / `getMonitor2CellModule(x,y)` 查询 monitor_2 表面模块/屏幕，复用 Monitor 的 `ModuleHandle` 体系**） |
