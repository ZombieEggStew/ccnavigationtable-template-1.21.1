...
| `block/MonitorSlabBlock.java` / `block/MonitorSlabBlockEntity.java` | 板式监视器（`ccpe:monitor_slab`，**独立方块**模式）：台阶状方块（FACE 地板/天花板/墙面 + 水平 FACING 四向 = 12 态，blockstate 结构参考 fmc/stock_link，**本阶段仅地板**；**可悬空放置**——无稳固检测，`canSurvive`/`neighborChanged` 已删，支撑方块破坏不掉落）；选择框 16×8×16 台阶盒；BE = `MonitorGridHost`（**14×14 表面棋盘网格** `GridState(14,14)`，面板 = slab 顶面 y8/16，网格起点 (1,1)px 格=1px，NBT 四路径 + writeSafe 蓝图兼容 + getUpdatePacket 不可恢复默认）——Monitor 的 8 个 payload、`MonitorModuleScreen` 模块配置、模块/屏幕共享渲染全部免费；**全局频道（信号系统）**：`channel`/`occupiedChannels` 字段，onLoad 注册 `GlobalChannelRegistry`（-1 自动分配最小空闲、冲突顺延，对齐 `MonitorBlockEntity`）、setRemoved 注销、`setChannel`（<0 忽略）/`refreshOccupiedChannels`，NBT 四路径存 `Channel`/`OccupiedChannels`（writeSafe 只存 Channel）；**CC:T 外设**：`getPeripheral()` 懒加载**直接复用 `MonitorPeripheral`**（`new MonitorPeripheral(this, "ccpe:monitor_slab")`，宿主参数化为 `MonitorGridHost`），capability 注册于 `CCPeripheralCapabilities`，`pe.getPeripheral(ch)` 按全局频道寻址；**扳手语义（照 ControlDeskBlock）**：`onWrenched` 一律消费**永不旋转**（配置菜单由 overlay 打开）、`onSneakWrenched` **不判定点击面**——`isSurfaceContentHit`（点击落点→网格格→查 grid 占用，模块/屏幕都算）放行拆单模块、已装内容禁止整拆（`gui.ccpe.monitor_slab.remove_blocked`）/光板整拆保数据、`useItemOn` 空手蹲下右键消费 | slab 朝向/碰撞/拆除/频道、blockstate `assets/ccpe/blockstates/monitor_slab.json`、模型 `assets/ccpe/models/block/monitor_slab/` |
| `client/MonitorSlabHitDetector.java` / `client/MonitorSlabGridOverlay.java` / `client/MonitorSlabClientRegistry.java` / `block/MonitorSlabRenderer.java` | slab 客户端：`MonitorSlabHitDetector`（射线 vs 水平面板平面 + 背面剔除 + 排除自身遮挡，FLOOR）；`MonitorSlabGridOverlay`（14×14 网格/预览/放置/按压/钮子/旋钮拖拽/屏幕两点放置/扳手拆单模块；**配置菜单打开在 `onClientTick`**——抄 ControlDeskPlacementOverlay，基于 `mc.hitResult` 命中 slab **任意位置（含侧面）** +（扳手普通右键 或 空手蹲下右键）→ 先判表面内容：模块/屏幕 → 留给 `onRenderLevel` 打开 `MonitorModuleScreen`（已验证逻辑），否则 → `MonitorSlabConfigScreen`；面板空格由 `onRenderLevel` 兜底；交互状态按 BlockPos 隔离）；`MonitorSlabClientRegistry`（已加载 slab 坐标集合）；`MonitorSlabRenderer`（BER：顶面叠加模块 + 9 宫格水平面 + 表面装饰） | slab 交互/命中/渲染 |
