...
| `block/EngineCoreBlockEntity.java` | 发动机核心 BE：`GeneratingKineticBlockEntity` 骨架，**当前占位不发电**（`getGeneratedSpeed()` = `GENERATED_SPEED` = 0，被动节点、贯通传动杆随网络连通）；后续接入燃烧/蒸汽模块时改转速并调 `updateGeneratedRotation()` | 发动机核心发电逻辑接入 |
...
| `block/CoolingDuctBlock.java` | 冷却风道（`ccpe:cooling_duct`）：**6 面贴附 × 每面 2 旋转 = 12 个 blockstate**（`FACING` + `ROLL`(0/1)，参考 PitotTubeBlock）；**双模型**：地板/天花板用平模型 `block.json`（y 旋转南北/东西，放置按玩家水平朝向定 ROLL），墙面用竖模型 `block_v.json`（风道竖直贴墙，绕 Y 转 4 次贴东南西北，roll=0/1 渲染相同）；纯静态方块（无 BE，物品模型 = block 模型）；IWrenchable：仅地板/天花板（竖直轴贴附面）右键顶面 ROLL 0↔1 切换、墙面不旋转、潜行右键拆除；音效对齐发动机核心（SoundType.NETHERITE_BLOCK） | 风道朝向/碰撞、blockstate `assets/ccpe/blockstates/cooling_duct.json`、竖模型 `assets/ccpe/models/block/aero_engine/cooling_duct/block_v.json` |
