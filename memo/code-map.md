...
| `block/EngineCoreBlockEntity.java` | 发动机核心 BE：`GeneratingKineticBlockEntity` 骨架，**当前占位不发电**（`getGeneratedSpeed()` = `GENERATED_SPEED` = 0，被动节点、贯通传动杆随网络连通）；后续接入燃烧/蒸汽模块时改转速并调 `updateGeneratedRotation()`；**静态工具 `engineControllerPos(level, pos)`**：从任一模块方块（核心成员/燃烧室/冷却风道）解析整条引擎 controller 坐标（未连接核心返回 null），供模块 goggle tooltip 代理 | 发动机核心发电逻辑接入 |
...
| `block/FluidCombustionChamberBlock.java` | 流体燃烧室（`ccpe:fluid_combustion_chamber`）：**6 向贴附式方块**（blockstate 结构照抄 fluid_port：未旋转变体 = facing=up、开口朝 FACING、放置 FACING = 点击面，共 6 个 state）；选择框 = 腔体 16×8 + 顶沿 collar（6 向）；活塞由 Flywheel/BER 叠加渲染；**IWrenchable 默认处理**（同 AIC/FMC：潜行右键拆除掉包、右键无旋转）；音效对齐发动机核心（SoundType.NETHERITE_BLOCK）；**实现 `IProxyHoveringInformation`**（goggle tooltip 代理到整条引擎 controller，与看核心共用同一 tooltip；未贴核心返回自身 → 不显示） | 燃烧室朝向/碰撞、blockstate `assets/ccpe/blockstates/fluid_combustion_chamber.json` |
...
| `block/SteamPowerChamberBlock.java` / `block/SteamPowerChamberBlockEntity.java` / `block/SteamPowerChamberVisual.java` / `block/SteamPowerChamberRenderer.java` | 蒸汽动力室（`ccpe:steam_power_chamber`）：**模式照抄流体燃烧室**（6 向贴附式 blockstate + 腔体静态渲染 + 活塞 Flywheel/BER 叠加，`getPistonOffset()` 占位 0；IWrenchable 默认扳手 + SoundType.NETHERITE_BLOCK；**实现 `IProxyHoveringInformation`** goggle tooltip 代理同流体室） | 与流体燃烧室相同 |
