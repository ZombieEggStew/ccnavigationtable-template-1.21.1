...
| `block/ControlDeskBlock.java` | 控制台方块：底座由 blockstate 静态模型渲染；控件安装/卸载/菜单交互（手持控件物品右键安装、扳手蹲下右键按点击位置拆单个模块、`getDrops` 破坏掉落、`onWrenched`/`useItemOn`（空手蹲下）一律消费右键供配置菜单打开——扳手不再旋转方块）；**棋盘网格自由放置**（monitor_2/throttle/joystick_2 无安装位框；`snappedBoxCenter` 命中点→1px 网格吸附中心，客户端预览与服务端放置共用；`joystick2PlaceBox` 放置盒世界 AABB（扳手拆除命中用）；`modelToWorld` 北向基准→世界旋转）；**拓展坞/挡板安装虚影（`DeskFormPlacementHelper`（ControlDeskBlock 内嵌类，DOCK+BAFFLE 共用，catnip PlacementClient/GhostBlocks 机制同 engine core/燃烧室））**——手持控制台方块物品（→DOCK）或 create:brass_casing（→BAFFLE）对准已放置的控制台显示半透明呼吸虚影（ghost state = `DOCKED=true` slab / `BAFFLED=true` 3/4 楼梯、保持 FACING，虚影位置 = 自身方块）；互斥判定同 `install(DOCK/BAFFLE)`（已装 DOCK/BAFFLE/PEDAL/JOYSTICK/JOYSTICK_3 → fail 不显示）；右键安装仍走 `useItemOn` DOCK/BAFFLE 分支，本助手只负责虚影渲染；选择框/碰撞箱 = 单块 `[0,0,8]~[16,8,16]`；`installBounds` 安装位 AABB（仅 PEDAL/JOYSTICK）+ `hitBounds` 闭区间容差命中（不能用 `AABB.contains`，点击位置在桌体面 z=8 = 框的 maxZ）；`FACING` 四向朝向 | 控制台朝向、碰撞、控件安装/卸载、菜单交互、放置位置计算、拆除命中、拓展坞虚影 |
...
| `block/EngineCoreBlock.java` | 发动机核心（`ccpe:engine_core`）：**Create 动力源方块骨架**（AXIS X/Y/Z 三向，blockstate 恰 3 个 state；模型南北对称，x/z 共用模型、y 用 x 旋转）；**贯通传动杆**（`hasShaftTowards` 沿 AXIS 前后两面，`getRotationAxis` = AXIS）；扳手默认处理（`RotatedPillarKineticBlock`→`IRotate`→`IWrenchable`：右键轴间切换、潜行右键拆除）；放置继承 `RotatedPillarKineticBlock.getStateForPlacement`（优先对齐相邻传动轴）；音效对齐 simulated:portable_engine（SoundType.NETHERITE_BLOCK）；**实现 `IProxyHoveringInformation`**（悬停任意核心节 → tooltip 源代理到整条 engine controller，代理时记录悬停方块 → 核心 tooltip；悬停非 controller 成员也显示整条引擎状态） | 发动机核心朝向/传动轴/交互、模型 `assets/ccpe/models/block/aero_engine/engine_core/` |
| `block/EngineCoreBlockEntity.java` | 发动机核心 BE：`GeneratingKineticBlockEntity` + `IMultiBlockEntityContainer`（P0 组网：controller/length/LastKnownPos/Uninitialized 重组，`getMaxLength=21`、宽度固定 1）；**P1–P3 controller tick**（扫描燃烧室 → 逐室评估 → 发电（转速 = 油门×256，定距桨单杆）/ 容量 = 室数×base×stress×油门 / 零缓存 capability drain / 牛顿冷却温度 + 过热滞回 + 冲压/气压冷却 / 油门同缩出力·热量·消耗·转速，非 controller `getGeneratedSpeed()` 恒 0）；**P4 Lua 外设 `ccpe:engine`**（`getPeripheral()` 非 controller 委托给 controller；内嵌 `Peripheral`：`getTemperature`/`isOverheated`/`getFluidTanks`/`setThrottle`/`getThrottle` + `attach`/`detach` 维护 `luaConnected`；throttle=0 停机（`setEnabled`/`getEnabled` 已移除，油门 = 唯一启停控制）；注册见 `compat/cc/CCPeripheralCapabilities`）；**P5 混合比**（`mixture` 字段默认 1.0 + 气压驱动自动富油 `autoRichness()`（P = `getPressureForEngine` 与冷却同曲线，K=0.45 → Y≈260≈×1.25）+ 凸热曲线 `heatFactor()`（稀侧加速惩罚/浓侧平缓下限 0.7）；Lua `getMixture`/`getEffectiveMixture`/`getAutoRichness`/`setMixture(0.6~1.4)`（getAutoRichness = 自然高度混合比，仅自动富油系数不含杆值，海拔补偿参考）；**P7 奖励驱动：油耗只随杆值×经济系数×过冷惩罚（自动富油不进油耗，只作用于发热 = 富油降温）；经济系数 = 双因素 AND 门控 + 时间解锁进度（|T−155|≤10 ∧ 0.8≤m_eff≤1.1 持续达标 → 15s 缓慢解锁 0.75，离开 6s 流失）；温度全部引擎固定（经济 155 / 过冷 100 / 过热 200，不随燃料不随油门）；过冷惩罚 cold=1+0.3×max(0,100−T)/100；过稀失火后续阶段**；应力/转速不动）；**P2.5 蒸汽燃料 + P2.5b 源抽取重构**（**四类源列表** `waterSources`/`fluidFuelSources`/`steamFuelSources`/`solidFuelSources` + 游标，事件化重建：onLoad / 方块 neighborChanged 去抖 10 tick（仅邻居带流体/物品能力触发，`onModuleNeighborChanged`）/ 连接性变化 → `markSourcesDirty`，全源失败 → `scheduleSourcesRescan`（延迟 20 tick）；**批量补料**：水/流体燃料储备 `waterReserve`/`fluidFuelReserve`（mb）空才补一批（批次 = 室数×配置倍数 K，`SOURCE_BATCH_MULTIPLIER` 进游戏缓存），头罐失败即时切下一个，储备耗尽同 tick 补料防应力闪断；固体并行燃烧 `tryPullSolidFuelBatch` 一次抽 N×K 个（当前箱不足切下一箱 simulate 先验）；蒸汽流体燃料 `refillSteamFluidFuel` 批量 mb → 每室 + 批次/N × burnTicksPerBucket/1000；**删 priority 按查找顺序抽**（EngineFuels 不再排序）；**储备倒计时独立于燃料源**（拆箱不停机）；**蒸汽运行 = 油门开启且 T≥100°C 且水储备>0**（无燃料余热也运转，容量 = 全部蒸汽室）；**蒸汽消耗 ∝ 油门**（burnTick 每秒烧 油门 个、水/流体同缩，真实节流阀）；Goggle 燃料行数据 `steamFuelType`/`steamFuelKey`/`steamBurnTicksRemaining`/`steamSolidFuelKey` NBT 同步，`steamFuelValueComponent` 显示 xN + 墙钟秒）；**静态工具 `engineControllerPos(level, pos)`**：从任一模块方块（核心成员/燃烧室/冷却气道）解析整条引擎 controller 坐标（未连接核心返回 null），供模块 goggle tooltip 代理；**客户端活塞"噗嗤"音效池 ×2**（`fluidPistonSoundPool`/`steamPistonSoundPool`，`SoundPool(4,2)` 懒创建，音量各自走 `Config.ENGINE_*_PUFF_VOLUME`，`tickClient` 每 tick `play()`，燃烧室按类型入队；音量=0 跳过）；**Goggle 按悬停方块分流四套 tooltip**（核心/模块方块 `getInformationSource` 代理时经 `markHoveredModule` 在 controller 记录 `hoveredSourceBlock`，`addToGoggleTooltip` 分流后复位）：引擎核心 = 温度行 + Lua 控制行 + 总应力输出行（NBT 同步 `moduleCapacity`）+ 目前转速行（`getGeneratedSpeed`）+ 连接的模块清单（`scanModule` 轻扫，`- 名称 x数量`）；蒸汽动力室 = 状态（停机/正常/暖机中）+ 温度 + 油门（=效率%）；整合气道 = 温度 + 效率（=风门 `setCooling`%）；流体燃烧室 = 全量引擎状态（状态/温度/效率/经济/风门/混合比/Lua 行，行为不变） | 发动机核心发电/冷却/混合比/Lua 控制逻辑 |
| `block/EngineCoreRenderer.java` / `block/EngineCoreVisual.java` | 发动机核心渲染：方块本体由 blockstate 静态模型渲染；**贯通传动杆 = AXIS 两端各一个 Create `SHAFT_HALF`**（BER：`CachedBuffers.partialFacing` + `kineticRotationTransform`；Flywheel：2× `RotatingInstance` + `rotateToFace(SOUTH, 端面)`，同轴同相旋转）；参考 GearboxRenderer/GearboxVisual、MyBearingRenderer/MyBearingVisual | 贯通传动杆渲染/旋转 |
| `block/FluidCombustionChamberBlock.java` | 流体燃烧室（`ccpe:fluid_combustion_chamber`）：**6 向贴附式方块**（blockstate 结构照抄 fluid_port：未旋转变体 = facing=up、开口朝 FACING、放置 FACING = 点击面，共 6 个 state）；选择框 = 腔体 16×8 + 顶沿 collar（6 向）；活塞由 Flywheel/BER 叠加渲染；**IWrenchable 默认处理**（同 AIC/FMC：潜行右键拆除掉包、右键无旋转）；音效对齐发动机核心（SoundType.NETHERITE_BLOCK）；**实现 `IProxyHoveringInformation`**（goggle tooltip 代理到整条引擎 controller，代理时记录悬停方块 → 流体室显示全量引擎 tooltip；未贴核心返回自身 → 不显示）；`getTicker` 仅客户端 tick（活塞"噗嗤"音效触发） | 燃烧室朝向/碰撞、blockstate `assets/ccpe/blockstates/fluid_combustion_chamber.json` |
| `block/FluidCombustionChamberBlockEntity.java` | 流体燃烧室 BE：轻量 `BlockEntity`（供 Visual/BER 读活塞状态）；**活塞动画随转速推进**（角度 = gameTime×speed×3/10 度/tick，1 圈 = 1 往复，照 Create SteamEngine；`getPistonOffset(partialTick)` 客户端按帧算）；**客户端 `tickClient()`**：活塞相位回绕（一周期一次）→ 入队引擎 controller 的流体音效池 `getFluidPistonSoundPool()` 触发"噗嗤"（音量配置 `ENGINE_FLUID_PUFF_VOLUME`，=0 跳过） | 燃烧室活塞动画/音效、转速挂钩 |
...
| `block/SteamPowerChamberBlock.java` / `block/SteamPowerChamberBlockEntity.java` / `block/SteamPowerChamberVisual.java` / `block/SteamPowerChamberRenderer.java` | 蒸汽动力室（`ccpe:steam_power_chamber`）：**模式照抄流体燃烧室**（6 向贴附式 blockstate + 腔体静态渲染 + 活塞 Flywheel/BER 叠加 + `getPistonOffset()` 随转速推进；IWrenchable 默认扳手 + SoundType.NETHERITE_BLOCK；**实现 `IProxyHoveringInformation`** goggle tooltip 代理同流体室，代理时记录悬停方块 → **蒸汽室显示专属精简 tooltip（状态：停机/正常/暖机中 + 温度 + 油门=效率%）**；`getTicker` 客户端音效 tick 同流体室，入队蒸汽音效池 `getSteamPistonSoundPool()`，音量配置 `ENGINE_STEAM_PUFF_VOLUME`） | 与流体燃烧室相同 |
| `block/QuickFillFluidTankBlock.java` / `block/QuickFillFluidTankBlockEntity.java` | 快速装填流体储罐（`ccpe:quick_fill_fluid_tank`）：**6 向贴附式方块**（blockstate 照抄 fluid_port 加 OPEN 开盖动画 10 tick 自关，默认闭盖）；带 BE = **4000mb 单槽流体存储**（NeoForge `FluidTank`，暴露 `FluidHandler.BLOCK` 能力注册于 `CCPeripheralExtender#registerBlockCapabilities`）；交互：手持流体容器存入 / 空桶装满 1 桶；**goggle tooltip 同 create:fluid_tank**（BE 实现 `IHaveGoggleInformation`）；**客户端同步 = 罐体变更回调**（匿名 FluidTank 覆写 `onContentsChanged` → `setChanged` + `sendBlockUpdated`，任何来源的存取/引擎 drain 都推送，参考 Create SmartFluidTank→onFluidStackChanged→sendData 模式）；IWrenchable 默认扳手；音效对齐 fluid_port（SoundType.COPPER） | 储罐朝向/碰撞、blockstate `assets/ccpe/blockstates/quick_fill_fluid_tank.json`、goggle 数据同步 |
| `block/QuickFillFuelVaultBlock.java` / `block/QuickFillFuelVaultBlockEntity.java` | 快速装填燃料箱（`ccpe:quick_fill_fuel_vault`）：**6 向贴附式台阶状方块**（blockstate 照抄 fluid_port 加 OPEN 开盖动画 10 tick 自关，默认闭盖）；带 BE = **单物品类型库存**（容量 1024，类型占位 count=1 + `storedCount` 分离，避开 ItemStack count byte 序列化坑）；交互：手持整组存入 / 空手自动存一组 / 空手蹲下取出一组；方块移除掉包；**暴露 `ItemHandler.BLOCK` 能力**（`getItemHandler()` 单槽视图，注册于 `CCPeripheralExtender#registerBlockCapabilities`）——供引擎蒸汽室固体燃料自动抽取（`EngineCoreBlockEntity#tryPullSolidFuelBatch`，并行燃烧一次抽 N×K 个、当前箱不足切下一箱）；Jade 集成 `QuickFillFuelVaultWailaPlugin` | 燃料箱存取交互、单物品库存、开盖动画、引擎固体燃料抽取 |
| `block/CoolingAirDuctBlock.java` | 冷却气道（`ccpe:cooling_air_duct`）：**6 面贴附 × 每面 2 旋转 = 12 个 blockstate**（`FACING` + `AXIS_ALONG_FIRST`(布尔)，结构参考 PitotTubeBlock / rope_connector 的 `AbstractDirectionalAxisBlock`）；**双模型**：地板/天花板/墙面 axis=true 用平模型 `block.json`（y 旋转出南北/东西、墙面叠 x:90 贴墙 = 第二旋转），墙面 axis=false 用竖模型 `block_v.json`（竖直贴墙，绕 Y 4 向）；放置：地板/天花板按玩家水平朝向定 axis、墙面恒 false；纯静态方块（无 BE，物品模型 = block 模型）；IWrenchable（照 rope_connector）：右键贴附面（= FACING）翻转 axis（任何朝向通用）、其他面走 Create 默认（换贴面/转朝向）、潜行右键拆除；音效对齐发动机核心（SoundType.NETHERITE_BLOCK）；**实现 `IProxyHoveringInformation`**（无 BE 也显示 goggle tooltip：经 `EngineCoreBlockEntity.engineControllerPos` 代理到整条引擎 controller，代理时记录悬停方块 → **气道显示专属精简 tooltip（温度 + 效率=风门 setCooling%）**；未连接核心返回自身 → 无 BE 不显示）；**纯散热模块（原整合气道改版）：setCooling/setMixture 均无门控（纯存值）；风门值只在装风道时被 K_DUCT 分量消费** | 风道朝向/碰撞、blockstate `assets/ccpe/blockstates/cooling_air_duct.json`、竖模型 `assets/ccpe/models/block/aero_engine/cooling_air_duct/block_v.json` |
| `block/MonitorSlabBlock.java` / `block/MonitorSlabBlockEntity.java` | 板式监视器（`ccpe:monitor_slab`，**独立方块**模式）：台阶状方块（FACE 地板/天花板/墙面 + 水平 FACING 四向 = 12 态，blockstate 结构参考 fmc/stock_link，**表面模块：地板 + 贴墙 + 贴天花板全部接入**；**可悬空放置**——无稳固检测，`canSurvive`/`neighborChanged` 已删，支撑方块破坏不掉落）；选择框 16×8×16 台阶盒；BE = `MonitorGridHost`（**14×14 表面棋盘网格** `GridState(14,14)`，网格起点 (1,1)px 格=1px，NBT 四路径 + writeSafe 蓝图兼容 + getUpdatePacket 不可恢复默认）；**面板几何单一来源 `panelFrame(BlockState)`**（`PanelFrame`：地板 = 顶面 y8/16、贴墙 = 朝向 FACING 的 8px 竖直边界、天花板 = 底面 y8/16（朝下），grid x = 面板水平向右 / grid y = 朝上 / 法线朝外，`panelLocalToWorld`；R_face = Ry(faceYaw)·Rx(faceXRot) 仅贴墙用（Rx(−90)），**天花板渲染用「地板渲染整体绕面板平面镜像」不走面板变换**——Rx(180) 面板变换会让局部 Z 变世界 −Z 导致偏移）——Monitor 的 8 个 payload、`MonitorModuleScreen` 模块配置、模块/屏幕共享渲染全部免费；**全局频道（信号系统）**：`channel`/`occupiedChannels` 字段，onLoad 注册 `GlobalChannelRegistry`（-1 自动分配最小空闲、冲突顺延，对齐 `MonitorBlockEntity`）、setRemoved 注销、`setChannel`（<0 忽略）/`refreshOccupiedChannels`，NBT 四路径存 `Channel`/`OccupiedChannels`（writeSafe 只存 Channel）；**CC:T 外设**：`getPeripheral()` 懒加载**直接复用 `MonitorPeripheral`**（`new MonitorPeripheral(this, "ccpe:monitor_slab")`，宿主参数化为 `MonitorGridHost`），capability 注册于 `CCPeripheralCapabilities`，`pe.getPeripheral(ch)` 按全局频道寻址；**扳手语义（照 ControlDeskBlock）**：`onWrenched` 一律消费**永不旋转**（配置菜单由 overlay 打开）、`onSneakWrenched` **不判定点击面**——`isSurfaceContentHit`（地板面板 / 贴墙+天花板面板 `isPanelSurfaceContentHit`，点击落点→面板局部→网格格→查 grid 占用，模块/屏幕都算）放行拆单模块、已装内容禁止整拆（`gui.ccpe.monitor_slab.remove_blocked`）/光板整拆保数据、`useItemOn` 空手蹲下右键消费 | slab 朝向/碰撞/拆除/频道、blockstate `assets/ccpe/blockstates/monitor_slab.json`、模型 `assets/ccpe/models/block/monitor_slab/` |
| `client/MonitorSlabHitDetector.java` / `client/MonitorSlabGridOverlay.java` / `client/MonitorSlabClientRegistry.java` / `block/MonitorSlabRenderer.java` | slab 客户端：`MonitorSlabHitDetector`（射线 vs 面板平面，按 `panelFrame` 取地板水平面 / 贴墙竖直边界 / 天花板底面，背面剔除 d·n<0 + 排除自身遮挡，FLOOR+WALL+CEILING）；`MonitorSlabGridOverlay`（14×14 网格/预览/放置/按压/钮子/旋钮拖拽/屏幕两点放置/扳手拆单模块，`world()` 按面板坐标系映射；**配置菜单打开在 `onClientTick`**——抄 ControlDeskPlacementOverlay，基于 `mc.hitResult` 命中 slab **任意位置（含侧面）** +（扳手普通右键 或 空手蹲下右键）→ 先判表面内容：模块/屏幕 → 留给 `onRenderLevel` 打开 `MonitorModuleScreen`（已验证逻辑），否则 → `MonitorSlabConfigScreen`；面板空格由 `onRenderLevel` 兜底；交互状态按 BlockPos 隔离）；`MonitorSlabClientRegistry`（已加载 slab 坐标集合）；`MonitorSlabRenderer`（BER：模块/9 宫格水平面 + 表面装饰；贴墙套面板变换 `Ry(faceYaw)·Rx(faceXRot)`（模块朝向 0、屏幕 inPlane=0、z()=凸出 1px）；**天花板 = 模块 `Rx(180)` + facing=−facingYRotation+180（枢轴不取反）**；**屏幕 = 反射 R_ref `Rx(90)·scale(1,1,−1)` + 垂直 ScreenPlane（z()=−1/16、inPlane=−facingInPlaneDeg、mirrorWinding）**——反射翻转绕序必须反转 quad 补偿（`ScreenPlane.mirrorWinding()` → `Screen9GridRenderer.renderModel` reverseWinding + `ScreenTextRenderer.glyphQuad` 换序；**禁止裸 scale(1,−1,1) 镜像（模型内外翻转）**） | slab 交互/命中/渲染 |
...
| `block/Screen9GridRenderer.java` | **屏幕 9 宫格 + 格子文本共享渲染（Monitor 与 monitor_2 共用）**：`ScreenPlane` 接口（originX/originY/z/cellSize + `horizontal()` 水平顶面摊平 + `inPlaneRotationDeg()` 平面内旋转）+ `renderScreen`/`renderScreenText`/`renderModel`，统一块单位——消除移植时「px/块单位只改一半」类 bug（monitor_2 曾把 width 错乘 16f）；**水平面只旋转内容（文字/图形）绕屏幕区域中心、9 宫格外框保持网格对齐**（monitor_slab 屏幕旋转 bug 修复：外框整体旋转会宽高互换/错位，如 4×5→5×4；文字绘制体抽为 `renderTextContent`，`renderScreenText` 与 `renderScreen` 共用） | 修改 9 宫格/屏幕文字绘制，Monitor 与 monitor_2 同步生效 |
...
| `screen/MonitorSlabConfigScreen.java` | slab 配置菜单（继承 `AbstractMonitorScreen`）：**抄 `ControlDeskConfigScreen` 但只保留第一行频道滚轮条**（已安装控件列表不适用：slab 表面模块配置走 `MonitorModuleScreen`）；频道走**全局频道系统**（`GlobalChannelRegistry`，与显示器/传感器共享命名空间，跳过已占用频道照 `MonitorMenuScreen`），关闭时经 `MonitorSlabChannelPayload` 保存；无物理体判断（全局频道恒可用） | slab 频道菜单布局/打开入口、语言文件 |
...
| `network/MonitorSlabChannelPayload.java` | 客户端 → 服务端 | 保存 monitor_slab 全局频道号（与显示器/传感器共享全局频道命名空间，处理器在 `MonitorPacketHandlers`） |
...
传感器、显示器与 **monitor_slab 板式监视器**共享同一全局频道命名空间（`compat/cc/GlobalChannelRegistry`，内部是同一个 `ChannelRegistry` 实例），保证频道全局唯一（`broadcastRefresh` 会刷新 PE 传感器 / Monitor / MonitorSlab 三者的占用快照）。短程信号链接器是**独立物理体作用域频道空间**（`compat/cc/ShortRangeLinkerRegistry`，见下），与全局空间完全隔离；**控制台频道也已迁出全局空间**：复用链接器的物理体作用域频道空间（同链内链接器与控制台共享频道号、冲突顺延）。
...
| `compat/cc/ThrottleModuleHandle.java` | 油门杆模块实例：前进/后退按住态 `isForwardActive/isBackActive` + 组合判断 `isActive`（前进或后退任一按住，读服务端输入租约）+ 档位 `getThrottleGear()`（0..11 整数，锁存不回正；自由模式下 = 就近档位）+ 轴值 `getAxis()`（0..1，档位模式离散 / 自由模式连续）+ 自由模式开关 `isFreeMode/setFreeMode`（mainThread=true，NBT 持久化，无 GUI）+ 直接写位置 `setAxis`（0..1，mainThread=true）；读取 mainThread=false |
...
| `compat/cc/MonitorPeripheral.java` | Monitor 的 `IPeripheral` 实现（模块/屏幕查询入口）；**宿主参数化为 `MonitorGridHost`（type 构造器传入）：Monitor 用 `"ccpe:monitor"`，monitor_2 经 `ControlDeskPeripheral.getModule("monitor")` 复用同一类（type = `"ccpe:monitor_2"`），monitor_slab 经 `MonitorSlabBlockEntity.getPeripheral()` 复用（type = `"ccpe:monitor_slab"`），`getCellModule(x,y)` / `getModule(id)` / 音效方法完全同款，作用在各自网格上** |
...
| 修改 monitor_slab 板式监视器（放置/交互/渲染/频道/配置菜单） | `block/MonitorSlabBlockEntity.java`（`MonitorGridHost` 14×14 + 全局频道）、`client/MonitorSlabGridOverlay.java`（交互/菜单打开）、`client/MonitorSlabHitDetector.java`（命中）、`block/MonitorSlabRenderer.java`（渲染）、`screen/MonitorSlabConfigScreen.java`（频道菜单）、`block/MonitorSlabBlock.java`（扳手语义） | `MonitorSlabChannelPayload`、`GlobalChannelRegistry`（占用广播）、`MonitorPacketHandlers`、语言文件、`memo/monitor-slab.md`（实施记录与坑） |
