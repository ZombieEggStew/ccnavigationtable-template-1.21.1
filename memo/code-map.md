...
| `screen/JoystickModuleScreen.java` / `screen/Joystick2ModuleScreen.java` / `screen/PedalModuleScreen.java` / `screen/ThrottleModuleScreen.java` | controlDesk 控件（模块）设置菜单（继承 `AbstractMonitorScreen`）：背景复用 MonitorModuleScreen（`MyUIElements.BACKGROUND` 192×169）；操纵杆/摇杆2 双按键绑定条 + 双滚轮条（摇杆2 配置独立，经 `Joystick2ConfigPayload` 持久化）、脚踏板按键绑定条 + 回正时间条、油门杆前进/后退按键绑定条（`DoubleInputBar`）+ 档位切换节奏条（`ScrollValueBar`）。**入口 = 控制台配置菜单中点击已安装控件行**（`InstalledModulesList` 点击回调按控件类型分发）；`withReturnTo(Screen)` 设置关闭后返回的上级菜单（配置菜单传入自身，模块菜单关闭后回到配置菜单） |
