...
- **安装预览**（`client/ControlDeskPlacementOverlay`，已注册）：手持控件物品 + 准星指向 controlDesk（原版 `mc.hitResult`）→ Catnip Outliner 在安装位显示预览框，绿=可装 / 红=已装；每 tick 重新 show，离开/换物品自动消失
  - **半透明模型预览**（`client/ControlDeskGhostPreviewRenderer`，已注册，参考 aeroworks `SocketPlacementClient#onRenderLevelStage`）：手持控件物品 + 准星指向 controlDesk 且该位置**可安装**（未装该控件）→ 在安装位渲染控件**半透明模型**（`RenderLevelStageEvent.AFTER_BLOCK_ENTITIES` + `CachedBuffers.partial` + `RenderType.translucentMovingBlock()` + `color(255,255,255,110)` 约 43% + 固定光照 `0xF000F0`；facing 旋转与 BER 同一约定 `rotateCenteredDegrees(-facing.getOpposite().toYRot())`）；与线框预览**共存**（对齐 aeroworks socket 线框 + ghost 模型方案），已装位置仍只显示红色线框；部件列表 = `ControlDeskRenderer` 安装渲染的底座→本体（PEDAL base+左右 / JOYSTICK base+杆 / MONITOR_2 单体 / THROTTLE base+手柄+指示灯 / JOYSTICK_2 base+手柄 / THROTTLE_2 base+手柄）；**Sable 物理体兼容**：子次元内用 `renderPose().transformPosition` 把 plot 坐标投影到世界再施加子次元朝向（参考 aeroworks `SubLevelPoseClient.translateTo`，踩坑记录见 `control-desk-grid-slot.md` 文末）
...
- **油门位置状态（档位/自由两模式）**：✅ `ControlDeskBlockEntity.throttlePx`（**float 0..MAX_TRAVEL_PX**，服务端权威：档位模式恒为整数 1px=1 档、自由模式连续可停在档位之间，默认最低档 0）+ 输入租约（前进/后退按住态）+ 充电计数 `throttleChargeTicks`（仅档位模式）；tickServer 模拟：**档位模式** 前进按住充电满 {@code TICKS_PER_GEAR}（4）tick 进一档 / 后退同样退一档 / 无输入锁存并清零充电 / 到顶底充电清零不动作，每个档位切换 `notifyChange` + 播放一次 `LEVER_CLICK`（`ThrottleMotion.pitchForGear` 音调随档位上升 0.75→1.5，最低档不响）；**自由模式**（`throttleFreeMode` = Lua `setFreeMode` 开启，无 GUI）按住每 tick 位移 = 1/档位切换节奏 px（满行程 = MAX × N tick，复用同一配置）、每跨过整数档位刻度（1px）播放一次 LEVER_CLICK（pitchForGear 音调随刻度上升、最低档 0 不响，与档位模式一致）、松开锁存；`getThrottleAxis()` = 位置/MAX（连续）经 `getUpdatePacket` 广播；同步规则同踏板轴（只写 `getUpdateTag`、客户端 `loadAdditional` 的 `contains` 守卫**连续读不取整**，不进 `saveAdditional`/`writeSafe`）；**模式开关是配置**：`ThrottleFreeMode` 走 NBT 四路径持久化（存档/蓝图保留，旧存档缺字段保持默认档位模式），客户端渲染据此分支（自由 = 平滑滑行、档位 = 张力段落感）
...
- **油门：手柄沿模型空间 x 轴平移（不是旋转）**：✅ 已实施（Visual（Flywheel）+ BER 双路径）。数值 = BE **运行时位置**（0..11px 服务端权威：档位模式离散整数 / 自由模式连续，均锁存不回正，经 getUpdatePacket 同步）；**动画 = 目标位置 + 操作者本地"张力蠕动"**（`ThrottleMotion.tensionPx`，**仅档位模式**）：按住前进/后退时把手向下一档方向**稍微移动**（≤ 1/3px，`SeatControlState.throttleDir` 驱动，仅联动控制台）；**张力充电进度由渲染层用帧时间平滑推进**（每帧 `frameTicks / TICKS_PER_GEAR` 累加，档位步进/按键边沿清零——避免游戏时间整 tick 跳变导致渲染卡顿），满 {@code TICKS_PER_GEAR}（4）tick 档位步进（客户端观察到档位变化清零）→ **张力清零 + `approachStep` 快速逼近（`STEP_DECAY=0.1`）突然到位**（参考 knob 卡位：吸附档位 + 前半程微扭动）；**自由模式**（Lua `setFreeMode` 开启，无 GUI）：**无张力**，渲染端用 `JoystickTilt.approach`（`SMOOTH_DECAY=0.3`）平滑指数逼近连续位置（对齐油门2/操纵杆滑行）；目标平移量 = 位置（档位模式 = 档位 × 1px；**模型默认 handle 在底端（-x 端），0 偏移起步**），平移方向 = **模型空间 x 轴**（BlockBench 中取的方向，随 FACING 旋转仍沿桌面方向）；指示灯 `ThrottleMotion.indicatorColor` 按**连续轴值**着色（档位/自由通用）
- **油门2：手柄绕枢轴 (4,2,8) 旋转（不是平移，总距杆类型）**：✅ 已实施（Visual（Flywheel）+ BER 双路径）。数值 = BE **连续角度** `throttle2Angle`（0..+30°，服务端权威，`simulateThrottle2` 每 tick 线性累加 30°/满偏tick（满偏时间 BE 配置，默认 20 tick），无输入：回正开关开启 → 按回正时间线性回**中位 15°**（`Throttle2Motion.NEUTRAL_DEG`），关闭 → **锁存不回正**——总距杆机械锁存）；**动画层**各渲染端用 `JoystickTilt.approach` 指数逼近追逐 `Throttle2Motion.targetDeg`（= BE 角度，单一实现，SMOOTHED 模式）；枢轴 **PIVOT (4,2,8)/16**（Blockbench 旋转中心，用户定稿，手柄绕**模型空间 z 轴**旋转——上抬 = 角度 +，方向符号待进游戏验证，反了翻转 `Throttle2Motion.targetDeg` 符号）；变换链 = 放置变换（平移到放置位 + 安装朝向旋转绕放置中心）+ 手柄绕枢轴旋转（模型空间最内层，Flywheel/BER 同链）
...
- ✅ Lua API（按定稿实现，直接读数值层 = BE 服务端权威状态，**对齐 Monitor 的「外设 → 模块实例」模式**；**Lua 全链路已进游戏验证通过**：`pe.getPeripheral(ch)` → `getModule("monitor")` → `MonitorPeripheral` → `getCellModule`/`getModule(id)` → 模块/屏幕 handle，屏幕字符/图形绘制正常）：
  ...
  - **模块实例上的状态读取方法全部 `mainThread=false`**（Lua 侧高频轮询直接跑 CC worker 线程，不占主线程）：
    ...
    - `ThrottleModuleHandle`：原始值 `isForwardActive()/isBackActive()`（boolean：前进/后退键按住态，读服务端输入租约）+ 组合判断 `isActive()`（前进或后退任一按住 = true，一次调用判断是否有人在操作）+ 档位 `getThrottleGear()`（0..11 整数，锁存不回正；自由模式下 = 就近档位）+ 轴值 `getAxis()`（0..1 = 位置/MAX，档位模式离散 / 自由模式连续，满前进 = 1）+ **自由模式开关 `isFreeMode()/setFreeMode(boolean)`（mainThread=true 服务端权威 + NBT 四路径持久化，无 GUI）** + **直接写位置 `setAxis(value)`（0..1 越界钳位，mainThread=true；自由模式连续写、档位模式吸附最近档位；玩家坐垫输入有效时每 tick 模拟覆盖，对齐 `Throttle2ModuleHandle#setAngle` 语义）**
...
- **油门杆配置已全部持久化**：前进/后退按键（`ThrottleKeyForward`/`ThrottleKeyBack`，默认 空格/左Ctrl，空串 = 未绑定）+ 档位切换节奏（`ThrottleTicksPerGear`，默认 4，范围 1..100）+ **自由模式开关（`ThrottleFreeMode`，默认关 = 档位模式；无 GUI，由 Lua `setFreeMode` 切换，蓝图/存档保留）** 存 BE NBT 四路径全覆盖；`ThrottleModuleScreen` 打开时读客户端 BE 初始化、`onClose` 经 `ThrottleConfigPayload`（4 字段，与操纵杆/踏板包分离防互覆盖）→ 服务端 setter（`setThrottleKeys`/`setThrottleTicksPerGear`；模式不经 GUI payload）
- **油门2 配置已全部持久化**：上抬/下拉按键（`Throttle2KeyUp`/`Throttle2KeyDown`，默认 空格/左Ctrl，空串 = 未绑定）+ 满偏时间（`Throttle2FreeSpeed`，默认 20，范围 1..100）+ 回正开关/回正时间（`Throttle2ReturnEnabled`，默认关 / `Throttle2ReturnTime`，默认 2，范围 0..100，0=关闭回正）存 BE NBT 四路径全覆盖；`Throttle2ModuleScreen` 打开时读客户端 BE 初始化、`onClose` 经 `Throttle2ConfigPayload`（6 字段，与油门/操纵杆/踏板包分离防互覆盖）→ 服务端 setter（`setThrottle2Keys`/`setThrottle2FreeSpeed`/`setThrottle2Return`）
...
8. ✅ 油门杆（throttle）档位逻辑 + 动画：写死按键（空格=前进 +x / 左Ctrl=后退 -x）→ `SeatInputPayload`（11 字段）→ 服务端校验 → BE 油门**档位**（0..11 离散，服务端权威，`getUpdatePacket` 广播）→ 手柄沿模型空间 x 轴平移（档位 × 1px，`approachStep` 快速逼近段落感）；每档切换 LEVER_CLICK 音效（音调随档位上升 0.75→1.5）；✅ 接配置 UI（`ThrottleModuleScreen`：`DoubleInputBar` 前进/后退 + `ScrollValueBar` 档位切换节奏 → `ThrottleConfigPayload` → BE NBT 持久化 + `SeatControlListener` 读 BE 配置驱动，渲染端张力充电同步跟随 BE 配置）；✅ **自由模式（不机械卡位、位置连续，每跨 1px 档位刻度播放一次卡位音效）已接入**：Lua `setFreeMode(true/false)` 切换（无 GUI）→ BE `throttleFreeMode`（NBT 四路径持久化 + updateTag 同步）→ 运行时位置 `throttlePx` float（档位模式整数 / 自由模式连续，`setAxis` 可直接写位置）→ 服务端每跨 1px 刻度播放 LEVER_CLICK（音调随刻度、最低档 0 不响）→ 渲染端按模式分支（自由 = `JoystickTilt.approach` 平滑滑行、档位 = 张力段落感）；⏳ 进游戏验证自由模式手感与方向
9. ✅ 摇杆2（joystick_2）输入检测 + 动画（照抄 joystick 模块，配置/轴值/输入租约全部独立）：`SeatControlListener` 收集 joystick_2 四向按键（默认 WASD）→ 同一 `SeatInputPayload` → 服务端校验写 `input2*` 租约 → `tickServer` 的 `simulateJoystick2` 用 joystick2 系列配置模拟 `joystick2AxisX/Y`（自由/档位模式）→ `getUpdatePacket` 广播 → Visual/BER 手柄绕枢轴 (8,1,8) 倾斜（15°、指数逼近，`Joystick2Motion` 单一实现）；✅ 配置 UI（`Joystick2ModuleScreen` 照抄 `JoystickModuleScreen` + `Joystick2ConfigPayload` → BE NBT 四路径持久化，入口 = 配置菜单点击摇杆2 行）；✅ 安装旋转加基础 +90° 偏移（`rotationToFace2`，预览/实装共用）；✅ Lua API（`getModule("joystick_2")` → `Joystick2ModuleHandle`，照抄 joystick、方法名对齐 joystick（无 2 前缀），读独立轴值）；⏳ 进游戏验证方向符号与枢轴手感（Lua 侧信号已随控制台 Lua 链路一并验证）
10. 🔶 油门2（throttle_2）输入检测 + 总距杆旋转动画：✅ 按键默认 空格=上抬 / 左Ctrl=下拉（独立输入字段 `inputThrottle2Up/Down`，`SeatInputPayload` 扩到 13 字段）→ 服务端校验写租约 → `tickServer` 的 `simulateThrottle2` 模拟连续角度（0..+30°，步进 30°/满偏tick、满偏时间 BE 配置默认 20 tick；回正开关开启时无输入按回正时间回中位 15°，关闭时锁存不回正）→ `getUpdatePacket` 广播 → Visual/BER 手柄绕枢轴 (4,2,8) 旋转（`Throttle2Motion` 单一实现，指数逼近）；✅ 配置 UI（`Throttle2ModuleScreen`：`DoubleInputBar` 上抬/下拉 + `ScrollValueBar` 满偏时间 + `ToggleButton`（RECOVER）回正开关/回正时间条 → `Throttle2ConfigPayload`（6 字段）→ BE NBT 四路径持久化，入口 = 配置菜单点击油门2 行）；✅ Lua API（`getModule("throttle_2")` → `Throttle2ModuleHandle`：`getAxis` 0..1 / `getCenterAxis` -1..1 / `setAngle` 角度控制）；⏳ 进游戏验证方向符号与枢轴手感
