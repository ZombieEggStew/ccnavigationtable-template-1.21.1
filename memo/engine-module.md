...
**Goggle（P7）**：状态行（含蒸汽「暖机中」、**流体「过冷」态（BLUE，cold>1.01）**）+ 温度行 + 「经济 ×」（**P7 显示最终油耗系数 = 杆值 × 经济系数 × 过冷惩罚**，双因素解锁进度自然体现在数值渐入）+ 「最佳温度 XX°C」（**随油门的当前目标值**，帮助玩家追窗口）+ 过冷提示行（cold>1 时「过冷 ×X.XX」）+「风门 XX%」+ 未装整合气道提示行——**经济/风门/混合比行仅流体引擎**（蒸汽无这些行）；**一律显示服务端同步值**（踩坑 10，客户端不重算 T_opt/系数）。
