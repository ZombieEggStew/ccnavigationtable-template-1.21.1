...
**Goggle（P7）**：状态行（含蒸汽「暖机中」）+ 温度行 + 「经济 ×0.75~1.0」（**双因素 + 解锁进度**：温度+混合比持续达标才渐入 0.75，掠过不奖励）+ 「最佳温度 XX°C」（**随油门的当前目标值**，帮助玩家追窗口）+ 过冷提示行（cold>1 时「过冷 ×X.XX」）+「风门 XX%」+ 未装整合气道提示行——**经济/风门/混合比行仅流体引擎**（蒸汽无这些行）；**一律显示服务端同步值**（踩坑 10，客户端不重算 T_opt/系数）。
