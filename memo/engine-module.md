...
> 设计封闭（用户逐项确认）：蒸汽室 = 水 + 燃料（无蒸汽流体）；**全引擎单燃料制**（priority 统一选一种，不混烧）+ **流体/蒸汽物理排斥**（不能混用）；严格零流体缓存；固体燃料从周围燃料箱（quick_fill_fuel_vault）自动抽取（**并行燃烧、流体优先**）；水走 drain 管线；**无红石控制，末段做 Lua 控制**；整合气道（散热+进气）替换冷却风道；**燃料体系分家**（蒸汽 = 纯原版解析，流体燃烧室 = 纯 datapack）；**温度阈值全部引擎固定**；过稀失火后续阶段。
...
| `steam_power_chamber`（蒸汽动力室） | 烧水 + 燃料产生蒸汽，**每室 3072 SU**；简单稳定、永不过热；燃料箱固体燃料并行燃烧 + 余热运转 |
...
- **蒸汽引擎 = 简单稳定**：燃料箱塞燃料 → 点火 → 暖机（<100°C 只烧不发电）→ 出力；**真实蒸汽车油门**（消耗 ∝ 油门，低油门省燃料）；**余热运转**（燃料耗尽 T≥100°C 仍满出力）；永不过热、高空无忧；无混合比/经济区/风门。
...
6. **蒸汽室**：消耗原料——水（1mb/s/室 × 油门）+ 燃料（原版 burnTime：固体从燃料箱**一次抽 N 个并行燃烧**；流体桶按熔炉燃烧时长；**流体优先**）。
...
| 固体燃料 | 从模块邻居燃料箱（quick_fill_fuel_vault 等 ItemHandler 容器）自动抽取（物品 burnTime，缓存源 + 冷却重扫，参考流体源罐模式）；**流体燃料优先**；**并行燃烧**：一次抽 N 个（N = 蒸汽室个数），不足 N 不抽，每室 1 个同燃同熄，燃烧时长 = 单个燃料时长 |
| 红石 | 不要；末段 Lua 控制 |
...
| 蒸汽温度 | 闭式锅炉自调节钉 `BOILER_T_OPT=155`；T<100°C 只烧不发电（暖机）；**运行 = 油门开启且 T≥100°C（无燃料余热也运转）**；**消耗 ∝ 油门（真实节流阀语义）**；永不过热、高度免疫；无经济区/混合比/风门 |
...
燃料：流体燃烧室 = engine_fuel/*.json（datapack）；蒸汽室 = 原版桶物品燃烧时长 + 固体物品 burnTime（燃料箱自动抽取）
```
...
- 逐室评估：流体室 = datapack 表第一个可用流体；蒸汽室 = 水可用且燃料可用（并行燃烧：全室同周期同燃同熄）。
...
- 固体燃料（P2.5）：水在才走；**流体燃料不可用** → `tryPullSolidFuelBatch` 从缓存的燃料箱（quick_fill_fuel_vault 等 ItemHandler 容器）**一次抽 N 个**（N = 蒸汽室个数，不足 N 不抽取），每室 +1 个物品 burnTime；燃料箱缓存逻辑 = 自动抽取流缓存储罐同款（源坐标 + 冷却重扫，`solidFuelSourcePos`/`solidFuelRescanCooldown`）；油门 0 不抽（停机不烧油）。
...
**P2.5 运行条件 = 油门开启且 T ≥ 100°C**（不再要求有燃料储备）：燃料耗尽但锅炉仍热时靠余热继续发电（容量 = 全部蒸汽室满出力），温度冷却到 100°C 以下才停机；燃烧倒计时独立于燃料源（燃料箱被拆储备烧完为止，见 7.3）。**消耗 ∝ 油门（真实蒸汽车节流阀语义）**：每秒烧 油门 个 burnTick + 水/流体同缩（25% 油门 = 1/4 蒸汽流量 = 燃料耐用 4 倍）；Goggle 燃料行显示**墙钟剩余秒 = burnTicks/(油门×20)**（与真实时间同步；油门 0 停机不显示倒计时）。无经济区/混合比/风门/整合气道需求。常量：`STEAM_MIN_WORK_TEMP=100`、`STEAM_WARMUP_RATE=0.25`（τ=4s）。
...
**固体并行燃烧**：一次抽 **N 个**（N = 蒸汽室个数）；**源不足 N 个不抽取**（先 `extractItem(slot, N, true)` simulate 校验，整组断供不拆零）；每室 +1 个物品 `burnTime`，各室**同燃同熄**——燃烧时长 = 单个燃料时长，全室空炉才批量补料（`tryPullSolidFuelBatch`）。
...
**缓存**：固体燃料源 = `solidFuelSourcePos` + `solidFuelRescanCooldown`（10 tick），参考自动抽取流缓存储罐（`steamFuelSourcePos`）同款模式（`solidFuelAvailable`）。

**燃烧倒计时独立于燃料源**：蒸汽室循环门控 = 水 OK 且（流体可用 || 固体可用 || 任一室 `burnTicks>0`）——燃料箱/源罐被拆时已有储备继续燃烧（不冻结不停机），倒计时结束才尝试再抽，抽不到才停烧（停烧 = 停止加热，温度开始牛顿冷却）。
...
| 水（蒸汽室） | **1mb/s/室 × 油门**（消耗 ∝ 蒸汽流量，25% 油门 = 0.25mb/s/室），从模块邻居水源罐 drain；水不可用整类暂停 |
| 固体燃料（蒸汽室） | 物品原版 `burnTime` → 每秒烧 **油门** 个 burnTick（25% 油门燃料耐用 4 倍）；**流体不可用**时从模块邻居燃料箱（quick_fill_fuel_vault 等）**一次抽 N 个并行燃烧**（N = 蒸汽室数，不足不抽，每室 1 个同燃同熄），流体优先 |
| 流体燃料（蒸汽室） | **纯原版解析**：桶物品原版熔炉燃烧时长（熔岩桶 20000 tick → 1mb 烧 20 tick × 油门）；`EngineFuels.vanillaBucketBurnTicks` 权威 |
...
6. 蒸汽室评估：门控 = 水OK 且（流体可用 || 固体可用 || 任一室有储备）；逐室 burnTick 消耗（×效率）；steamFuelBurnTicks 原版解析；**流体不可用 → 固体批量兜底**（全室空炉时一次抽 N 个，不足 N 不抽，并行燃烧）；**储备倒计时独立于燃料源**（源被拆仍烧完）
...
- **蒸汽引擎**：燃料箱塞燃料 → 点火 → 暖机（T 从环境爬到 100°C 约 4s，期间只烧不发电）→ 出力；**真实蒸汽车油门**：消耗 ∝ 油门（25% 油门燃料耐用 4 倍，Goggle 倒计时同步拉长），油门 0 = 停机不烧油；**余热运转**：燃料耗尽但 T≥100°C 仍满出力发电，冷却到 100°C 以下才停机；**并行燃烧**：N 个蒸汽室同时烧 N 个燃料（一次抽 N 个、不足不抽）；永不过热、高空无忧；代价 = 应力低（3072/室）。
...
- 固体燃料**不足 N 不抽**（simulate 先验）；并行补料时机 = 全室空炉
...
| P2.5（蒸汽燃料定稿） | 蒸汽固体燃料并行燃烧（一次抽 N、不足不抽、流体优先）+ 燃料箱 ItemHandler 能力 + 储备倒计时独立于燃料源（拆箱不停机）+ 余热运转（运行 = 油门+T≥100）+ 真实蒸汽车节流阀消耗（∝油门 + 墙钟显示）+ Goggle 燃料行（xN + 墙钟秒） | ✅ |
