...
**运行条件**（全满足才发电）：≥1 运行燃烧室 && 效率>0 && 未过载 && 未过热（仅流体）&& 蒸汽 T≥100°C 且水可用。停机原因：缺燃料/缺水、油门 0、过载、流体过热（滞回 T≤200 恢复）、蒸汽暖机未完成。
...
- 过载 = 停烧（不白烧油）；过热锁存（流体）T≤200 才恢复；高频启停（油门 0↔1）触发 Create flicker 惩罚。
