...
```
1. 枚举：scanModule —— 两类燃烧室（FACING 反面贴核心归属，去重）+ 整合气道数 D + 去重邻居（方案 A：每 tick 枚举当安全网）
...
**运行条件**（全满足才发电）：≥1 运行燃烧室 && 效率>0 && 未过载 && 未过热（仅流体）&& 蒸汽 T≥100°C 且水可用。停机原因：缺燃料/缺水、油门 0、过载、流体过热（滞回 T≤180 恢复）、蒸汽暖机未完成。
