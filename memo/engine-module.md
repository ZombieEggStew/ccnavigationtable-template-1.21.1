...
**P2.5 运行条件 = 油门开启且 T ≥ 100°C**（不再要求有燃料储备）：燃料耗尽但锅炉仍热时靠余热继续发电（容量 = 全部蒸汽室满出力），温度冷却到 100°C 以下才停机；燃烧倒计时独立于燃料源（燃料箱被拆储备烧完为止，见 7.3）。**消耗固定（蒸汽"无折扣"）**：1 个 burnTick 烧 1 tick（原版熔炉速率）+ 水 1mb/s/室，均不随油门缩放——油门只缩出力（容量/转速），Goggle 燃料行剩余秒数 = burnTicks/20 即真实墙钟秒数。无经济区/混合比/风门/整合气道需求。常量：`STEAM_MIN_WORK_TEMP=100`、`STEAM_WARMUP_RATE=0.25`（τ=4s）。
...
| 固体燃料（蒸汽室） | 物品原版 `burnTime` → 1 tick 烧 1 burnTick；**流体不可用**时从模块邻居燃料箱（quick_fill_fuel_vault 等）**一次抽 N 个并行燃烧**（N = 蒸汽室数，不足不抽，每室 1 个同燃同熄），流体优先 |
...
温度：XX.X°C
油门：50%
燃料：熔岩 / 煤炭 x3 (1m 20s) / 无      ← 参考 simulated portable_engine：流体只显示名称不显示时间；固体并行燃烧 = 数量 xN（N = 蒸汽室个数）+ 单个燃料剩余时间（burnTicks÷20 秒，getTime 格式，英文括号无"剩余"）；无储备显示红"无"
```
