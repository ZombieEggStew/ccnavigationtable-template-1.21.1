...
- [x] 验证项：setGrid 格子文本 / 光标 / 覆盖 / draw(batch) 整屏替换 / drawCells / drawShapes / fill 进度条 / 图形层自由定位 / 多屏幕并存。
...
- Lua API：新增 `setGrid/getGrid`、`fill`、`draw(batch)`（cells+shapes 两段式，原子替换）、`drawCells(batch)`/`drawShapes(batch)`（单层替换，另一层保持不变）；`setCursorPos` 改格子坐标（1 起）；`write` 去掉 z 参数；`setTextScale` 保留为 setGrid 别名（可传可选高宽比参数）。
