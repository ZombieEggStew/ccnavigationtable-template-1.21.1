...
- 新方块 `ccpe:my_aero_sensor`「惯性导航系统」：物理体姿态指示器（滚转/俯仰/偏航指北），视觉照抄 `simulated:gimbal_sensor` 的重力摆动画，进游戏验证通过。
...
- 服务端 `XAngle/ZAngle` getter 已预留，未来可接 `ccpe.sensor_system` Lua。详见 `memo/my_aero_sensor.md`。
