...
- [x] 屏幕渲染重构（方案三：格子模型）已实施，待进游戏验证；旧 Lua 程序文本定位部分需按新 API 改写（破坏性变更，见 `memo/screen-render-rework.md`）。
- [x] 验证项：setGrid 格子文本 / 光标 / 覆盖 / draw(batch) 整屏替换 / drawCells / drawShapes / writeField 定宽字段 / fillField 定宽填充 / fill 进度条 / 图形层自由定位 / 多屏幕并存。
