 ...
 *   <li>{@link Joystick3ConfigPayload} — 客户端→服务端：保存操纵杆3 配置（字段与操纵杆相同，独立于 joystick，原始操纵杆换皮版）</li>
...
public final class ControlDeskPacketHandlers {
    ...
    public static void register(PayloadRegistrar registrar) {
        ...
        // 客户端→服务端：保存 controlDesk 操纵杆3 配置（独立于操纵杆，服务端权威 + 落盘；原始操纵杆换皮版）
        registrar.playToServer(
                Joystick3ConfigPayload.TYPE,
                Joystick3ConfigPayload.STREAM_CODEC,
                (payload, ctx) -> {
                    var be = PacketHelper.findBE(ctx.player().level(), payload.pos(), ControlDeskBlockEntity.class);
                    if (be != null) {
                        be.setJoystick3ReturnTime(payload.returnTime());
                        be.setJoystick3ReturnTimeYaw(payload.returnTimeYaw());
                        be.setGear3Config(payload.gearModePitch(), payload.gearCountPitch(),
                                payload.gearModeYaw(), payload.gearCountYaw());
                        be.setJoystick3FreeSpeed(payload.freeSpeedPitch(), payload.freeSpeedYaw());
                        be.setJoystick3Keys(payload.keyUp(), payload.keyDown(), payload.keyLeft(), payload.keyRight());
                    }
                }
        );
