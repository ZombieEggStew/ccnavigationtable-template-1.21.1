 ...
 * 的轴值 -1..1，服务端权威）= 踩下键按住按满偏时间线性累加（{@link JoystickTilt#pressStep}，
 * 满偏 tick 数可配置，默认 2）、抬起键按住向 -1 累加、都不按按回正时间线性归零
 * （{@link JoystickTilt#returnStep}）；**动画**（渲染层）= 各渲染端用
 * {@link JoystickTilt#approach} 指数逼近追逐轴值 × {@link #MAX_TRAVEL}
