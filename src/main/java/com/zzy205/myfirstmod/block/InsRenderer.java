 ...
 */
public class InsRenderer extends SafeBlockEntityRenderer<InsBlockEntity> {

    /** 转动部件（万向环/罗盘盘/偏航标记）整体下移量（块单位）：模型与旋转中心同步下移 3.5px */
    private static final float PIVOT_DROP = 3.5f / 16f;

    public InsRenderer(final BlockEntityRendererProvider.Context context) {
    }

    @Override
    protected void renderSafe(final InsBlockEntity be, final float partialTicks, final PoseStack ms,
        ...
        this.apply(MyModPartialModels.INS_YAW, be, testQ, light, ms, vb);
        ...
        this.apply(MyModPartialModels.INS_GIMBAL, be, gimbalQ, light, ms, vb);
        ...
        this.apply(MyModPartialModels.INS_COMPASS, be, compassQ, light, ms, vb);
    }

    private void apply(final PartialModel model, final InsBlockEntity te, final Quaternionf Q,
