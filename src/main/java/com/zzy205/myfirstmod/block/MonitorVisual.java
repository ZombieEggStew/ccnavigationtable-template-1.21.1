...
public class MonitorVisual extends AbstractBlockEntityVisual<MonitorBlockEntity>
    ...
    private void transformAll() {
        ...
        this.applyShell(this.bearing, facing, yaw, offset, 0f, state.getValue(MonitorBlock.HANGING));
        this.applyShell(this.shell, facing, yaw, offset, pitch, false);
        ...
        for (MonitorModule mod : grid.getAllModules().values()) {
            ...
            this.applyShell(mv.base, facing, yaw, offset, pitch, false);
            ...
            this.applyShell(mv.extra, facing, yaw, offset, pitch, false);
     ...
     *  hanging = 挂顶翻转：模型空间最内层（先于 yaw/offset/facing 作用于顶点），与 BER 的 applyHanging 同链。 */
    private void applyShell(TransformedInstance instance, Direction facing, float yaw, int offset, float pitch, boolean hanging) {
