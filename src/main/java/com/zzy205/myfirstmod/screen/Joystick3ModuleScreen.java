...
import com.zzy205.myfirstmod.network.Joystick3ConfigPayload;
...
/**
 * 操纵杆3 设置菜单 —— 布局与 {@link JoystickModuleScreen} 完全相同（照抄），配置独立于 joystick
 * （{@code joystick3} 系列 BE 字段 / {@link Joystick3ConfigPayload}，与 joystick 同安装位互斥但配置各自独立）。
 * 打开方式：控制台配置菜单中点击已安装的操纵杆3 行（由 {@link ControlDeskConfigScreen} 分发）。
 * 布局（自上而下）：
 * <ol>
 *   <li>前后键位绑定条（W/S）</li>
 ...
 */
public class Joystick3ModuleScreen extends AbstractMonitorScreen {
    ...
    // 档位模式与回正时间的默认值/范围常量统一在 ControlDeskBlockEntity 定义（joystick3 系列）
    ...
    public Joystick3ModuleScreen(BlockPos deskPos) {
        super(Component.empty());
        this.deskPos = deskPos;
    }

    /** 设置关闭后返回的上级菜单（链式）；由控制台配置菜单打开时传入，关闭后回到配置菜单而非游戏。 */
    public Joystick3ModuleScreen withReturnTo(Screen returnScreen) {
    ...
    protected void init() {
        ...
        int returnTime = ControlDeskBlockEntity.DEFAULT_JOYSTICK3_RETURN_TIME;
        int returnTimeYaw = ControlDeskBlockEntity.DEFAULT_JOYSTICK3_RETURN_TIME;
        boolean gearModePitch = false;
        int gearCountPitch = ControlDeskBlockEntity.DEFAULT_JOYSTICK3_GEAR_COUNT;
        boolean gearModeYaw = false;
        int gearCountYaw = ControlDeskBlockEntity.DEFAULT_JOYSTICK3_GEAR_COUNT;
        int freeSpeedPitch = ControlDeskBlockEntity.DEFAULT_JOYSTICK3_FREE_SPEED;
        int freeSpeedYaw = ControlDeskBlockEntity.DEFAULT_JOYSTICK3_FREE_SPEED;
        String keyUp = ControlDeskBlockEntity.DEFAULT_JOYSTICK3_KEY_UP;
        String keyDown = ControlDeskBlockEntity.DEFAULT_JOYSTICK3_KEY_DOWN;
        String keyLeft = ControlDeskBlockEntity.DEFAULT_JOYSTICK3_KEY_LEFT;
        String keyRight = ControlDeskBlockEntity.DEFAULT_JOYSTICK3_KEY_RIGHT;
        if (this.minecraft != null && this.minecraft.level != null
                && this.minecraft.level.getBlockEntity(deskPos) instanceof ControlDeskBlockEntity desk) {
            returnTime = desk.getJoystick3ReturnTime();
            returnTimeYaw = desk.getJoystick3ReturnTimeYaw();
            gearModePitch = desk.isGear3ModePitch();
            gearCountPitch = desk.getGear3CountPitch();
            gearModeYaw = desk.isGear3ModeYaw();
            gearCountYaw = desk.getGear3CountYaw();
            freeSpeedPitch = desk.getJoystick3FreeSpeedPitch();
            freeSpeedYaw = desk.getJoystick3FreeSpeedYaw();
            keyUp = desk.getJoystick3KeyUp();
            keyDown = desk.getJoystick3KeyDown();
            keyLeft = desk.getJoystick3KeyLeft();
            keyRight = desk.getJoystick3KeyRight();
        }
    ...
    public void onClose() {
        ...
        PacketDistributor.sendToServer(new Joystick3ConfigPayload(deskPos,
    ...
    protected void renderCustom(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        // 标题：控件名
        g.drawString(this.font, Component.translatable("item.ccpe.joystick_3"),
