...
/**
 * 传感器频道注册表 —— 维护 频道 → MySensorBlockEntity 的一对一映射。
 * <p>
...
public final class SensorRegistry {
    ...
    /** 频道 → 传感器方块实体 */
    private static final Map<Integer, MySensorBlockEntity> registry = new ConcurrentHashMap<>();
     ...
     */
    public static int assignChannel(MySensorBlockEntity be) {
        cleanupZombies();
        int channel = 0;
     ...
     * @return 实际分配的频道号
     */
    public static int register(int channel, MySensorBlockEntity be) {
        // 清除该 BE 在其他频道上的旧条目（chunk 重载导致同一 BE 多频道残留）
        removeExistingEntryFor(be, channel);

        // 目标频道上的现有条目检查
        MySensorBlockEntity existing = registry.get(channel);
    ...
    private static int removeExistingEntryFor(MySensorBlockEntity be, int exceptChannel) {
        int[] count = {0};
     ...
     */
    public static void unregister(int channel, MySensorBlockEntity be) {
        MySensorBlockEntity existing = registry.get(channel);
        if (existing == be) {
            registry.remove(channel);
            notifyAllToRefresh();
    ...
    private static void notifyAllToRefresh() {
        for (MySensorBlockEntity be : List.copyOf(registry.values())) {
            try { be.refreshOccupiedChannels(); } catch (Exception ignored) {}
        }
    }

    public static MySensorBlockEntity get(int channel) {
