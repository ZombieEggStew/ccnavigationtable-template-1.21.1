 ...
 * <p>
 * 直接读 BE 数值层（服务端权威角度 0..+30°）：轴值 = 角度 / 满偏（0..1，0 = 最底端 / 1 = 满偏上台），
 * 回正模式专用轴值 = (角度 − 中位 15°) / 15°（-1..1，-1 = 底端 / 0 = 中位 / +1 = 满偏）。
 * 读取全部 {@code mainThread=false}（Lua 侧高频轮询直接跑 CC worker 线程，不占游戏主线程）；
 * 控制方法 {@link #setThrottle2Angle(double)} 为 {@code mainThread=true}（服务端权威写 BE 角度并广播）。
 */
public class Throttle2ModuleHandle {
    ...
    /**
     * 油门2 轴值（0..1）= 角度 / 满偏角（+30°）：0 = 最底端（放置默认），1 = 满偏上台。
     *
     * <pre>{@code
     * local th2 = desk.getModule("throttle_2")
     * print(th2.getThrottle2Axis())  -- 0.0 .. 1.0
     * }</pre>
     */
    @LuaFunction
    public final double getThrottle2Axis() {
        return be.getThrottle2Angle() / Throttle2Motion.MAX_DEG;
    }

    /**
     * 回正模式专用轴值（-1..1）= (角度 − 中位 15°) / 15°：
     * -1 = 最底端（0°），0 = 中位（15°，回正模式松开后的落点），+1 = 满偏上台（30°）。
     *
     * <pre>{@code
     * print(th2.getThrottle2CenterAxis())  -- -1.0 .. 1.0
     * }</pre>
     */
    @LuaFunction
    public final double getThrottle2CenterAxis() {
     ...
     *
     * <pre>{@code
     * th2.setThrottle2Angle(20)  -- 把手转到 20°
     * }</pre>
     *
    ...
    public final boolean setThrottle2Angle(double degrees) {
