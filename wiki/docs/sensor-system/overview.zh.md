...
**航电系统**是一组可安装在物理体（Sable sub-level）上的航电方块。装到物理体上之后，**同一物理体**（含约束链）上的 CC:Tweaked 电脑即可通过同一个 Lua 模块读取传感器数据：`require("ccpe.sensor_system")`。
...
| [飞行管理计算机](fmc.zh.md) | `ccpe:fmc` | 质量、重力、重心、附着方块的 Create 应力网络、螺旋桨转速求解器、高度-气压换算、风帆气动、通用阻力与最高高度求解工具 | ≥ 1 个 FMC |
