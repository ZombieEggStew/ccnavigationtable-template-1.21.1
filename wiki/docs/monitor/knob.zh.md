...
- **显示档位**：在配置界面中可以开启显示档位功能，显示当前旋钮的 角度 / 设定的卡位角度
- **物理限位**：在配置界面中可以开启物理限位功能，旋钮旋转到设定角度时会被夹紧，无法继续旋转
- **显示百分比**：在配置界面中可以开启显示百分比功能，显示当前旋钮的 角度 / 设定的最大旋转角度 × 100
---
...
返回相对比例（数字，0..1）：归一化角度 / 设定的最大旋转角度。

```lua
-- 最大旋转角度 360°，旋钮在 180°：返回 0.5
print(knob.getRelativePercent())
```

## knob.getAbsolutePercent()

返回绝对比例（数字）：绝对角度 / 设定的最大旋转角度。未开启物理限位时旋钮可转出设定范围，返回值可能超过 1 或为负数。

```lua
-- 最大旋转角度 360°，旋钮在 405°：返回 1.125
